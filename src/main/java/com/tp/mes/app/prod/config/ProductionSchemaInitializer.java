...
public class ProductionSchemaInitializer {
    ...
    public void init() {
        ...
        initTable("tp_notice",
                "create table tp_notice (" +
                        ...
                        "  constraint fk_tp_notice_created_by foreign key (created_by) references app_user(user_id)" +
                        ")");

        // 7. Alter Equipment (Add Util columns if missing)
        addEquipmentColumns();

        log.info("Production schema check complete.");
    }

    private void addEquipmentColumns() {
        try {
            // Check if column exists, if not add it. Oracle throws error if exists, so we
            // try-catch or check cols
            // Simplest: Try add, ignore specific ORA error (01430: column being added
            // already exists in table)
            try {
                jdbcTemplate.execute("ALTER TABLE equipment ADD (last_start_time timestamp)");
            } catch (DataAccessException e) {
                /* Ignore if exists */ }

            try {
                jdbcTemplate.execute("ALTER TABLE equipment ADD (total_run_time number default 0)");
            } catch (DataAccessException e) {
                /* Ignore if exists */ }

        } catch (Exception e) {
            log.warn("Failed to alter equipment table: {}", e.getMessage());
        }
    }
    ...
    private void initTable(String tableName, String ddl) {
        try {
            ...
            if ("equipment".equalsIgnoreCase(tableName)) {
                seedEquipment();
            } else if ("materials".equalsIgnoreCase(tableName)) {
                seedMaterials();
            } else if ("product_bom".equalsIgnoreCase(tableName)) {
                seedBom();
            } else if ("tp_notice".equalsIgnoreCase(tableName)) {
                seedNotice();
            }
    ...
    private void seedMaterials() {
        try {
            ...
            if (count != null && count == 0) {
                log.info("Seeding materials data...");
                jdbcTemplate.execute(
                        "INSERT INTO materials (material_id, material_name, current_stock, safe_stock_qty) VALUES ('MAT-001', 'Aluminum Sheet', 1000, 100)");
                jdbcTemplate.execute(
                        "INSERT INTO materials (material_id, material_name, current_stock, safe_stock_qty) VALUES ('MAT-002', 'Steel Rod', 500, 50)");
                log.info("Seeding materials complete.");
            }
        } catch (DataAccessException e) {
            log.warn("Failed to seed materials: {}", e.getMessage());
        }
    }

    private void seedBom() {
        try {
            Integer count = jdbcTemplate.queryForObject("SELECT count(*) FROM product_bom", Integer.class);
            if (count != null && count == 0) {
                log.info("Seeding BOM data...");
                // Assuming PROD-001 exists or will exist in production_plan as item_code
                jdbcTemplate.execute(
                        "INSERT INTO product_bom (product_item_code, material_inventory_id, quantity_required) VALUES ('PROD-ITEM-001', 'MAT-001', 2.5)");
                log.info("Seeding BOM complete.");
            }
        } catch (DataAccessException e) {
            log.warn("Failed to seed BOM: {}", e.getMessage());
        }
    }

    private void seedNotice() {
        try {
            Integer count = jdbcTemplate.queryForObject("SELECT count(*) FROM tp_notice", Integer.class);
            if (count != null && count == 0) {
                log.info("Seeding notices...");
                jdbcTemplate.execute(
                        "INSERT INTO tp_notice (title, body, created_at) VALUES ('Welcome to MES', 'System initialization complete. Welcome to the Manufacturing Execution System.', SYSTIMESTAMP)");
            }
        } catch (Exception e) {
            log.warn("Seed notice failed: {}", e.getMessage());
        }
    }
}
